...
describe SampleCardImageUploader do
  describe '#upload' do
    ...
    let(:s3_bucket) { s3_resource.bucket(Rails.configuration.s3.buckets[:cards]) }
    ...
    before do
      ...
      stub_const("#{described_class}::BUCKET_NAME", Rails.configuration.s3.buckets[:cards])
    ...
    context '画像作成が成功した場合' do
      ...
      it 'Terminalに画像作成成功メッセージを出力する' do
        expect { subject }.to output("[CREATED] #{Rails.configuration.s3.buckets[:cards]}/#{card.id}\n").to_stdout
      end
    end
    ...
    context 'card bucketが既に存在している場合' do
      before { s3_resource.create_bucket(bucket: Rails.configuration.s3.buckets[:cards]) }
